...
void
CameraSample::Take(std::optional<CameraSample> &maybe_sample)
{
	if (!this->has_sample) {
...
void
CameraSample::Push(
    t_blob_observation &blobservation,
...
void
CameraSample::Push(t_blob_observation &blobservation)
{
	this->blobservation = blobservation;
	this->num_devices_needing_slow_processing = 0;
...
Camera::DeferSampleToSlowThread(CameraSample &sample)
{
	...
	this->slow_processing_thread_data.sample.Push(sample.blobservation, sample.needs_slow_processing,
                      ...
                      struct xrt_pose &Tcv_world_device_candidate)
{
	...
	if (POSE_HAS_FLAGS(&score, POSE_MATCH_GOOD | POSE_MATCH_LED_IDS)) {
		this->PushPose(device, score, Tcv_cam_device_candidate, sample.blobservation, true);
		return true;
	}
                              ...
                              struct xrt_pose &Tcv_world_device_prior)
{
	...
	if (!ransac_pnp_pose(tracker->log_level, &Tcv_cam_device, tbo.blobs, tbo.num_blobs, &device->params.led_model,
	                     device->id, &this->model, NULL, NULL)) {
	...
	pose_metrics_evaluate_pose_with_prior(&score, &Tcv_cam_device, true, &Tcv_cam_device_prior,
	                                      &device->prior_pos_error, &device->prior_rot_error, tbo.blobs,
	                                      tbo.num_blobs, &device->params.led_model, device->id, &this->model, NULL);
	...
	if (POSE_HAS_FLAGS(&score, POSE_MATCH_GOOD)) {
		CT_DEBUG(tracker, "Camera %p RANSAC-PnP recovered pose for device %d from %u blobs", (void *)this,
		         device->id, tbo.num_blobs);
		this->PushPose(device, score, Tcv_cam_device, tbo, false);
		return true;
	}
...
Camera::SlowSampleProcess(CameraSample &sample)
{
	...
	CT_TRACE(tracker, "Starting slow processing for camera %p with %u blobs", (void *)this, tbo.num_blobs);
	...
	for (std::unique_ptr<Device> &device : tracker->devices) {
		...
		if ((search_flags & CS_FLAG_HAVE_POSE_PRIOR) != 0) {
			...
			if (auto Txr_world_cam = this->GetWorldPose(tbo.timestamp_ns)) {
		...
		if (correspondence_search_find_one_pose(data.cs, search_model, search_flags, &Tcv_cam_device,
			...
			pose_metrics_match_pose_to_blobs(&Tcv_cam_device, tbo.blobs, tbo.num_blobs,
			                                 ...
			                                 &blob_match_info);
			tracker->MarkMatchingBlobs(tbo, device->params.led_model, device->id, blob_match_info);

			t_blobwatch_mark_blob_device(tbo.source, &tbo, device->id);

			this->PushPose(device, score, Tcv_cam_device, tbo, true);
		}
	}
}
...
Camera::FastSampleProcess(CameraSample &sample)
{
	...
	CT_TRACE(tracker, "Starting fast processing for camera %p with %u blobs", (void *)this,
	         sample.blobservation.num_blobs);
	...
	auto Txr_world_cam = this->GetWorldPose(sample.blobservation.timestamp_ns);
	...
	for (std::unique_ptr<Device> &device : tracker->devices) {
		...
		if ((device_prior_relation.relation_flags & XRT_SPACE_RELATION_POSITION_VALID_BIT) != 0 &&
		    ...
		    this->TryDevicePose(device, sample, Tcv_cam_world, Tcv_world_device, Tcv_world_device)) {
		...
		if (has_last_known &&
		    this->TryDevicePose(device, sample, Tcv_cam_world, Tcv_world_device, Tcv_world_device_last_known)) {
			CT_DEBUG(tracker, "Fast processing for device %d succeeded with last known pose", device->id);
			continue; // try the next device, we found a pose!
		}

		if (this->TryDeviceBlobRecovery(device, sample, Tcv_cam_world, Tcv_world_device)) {
...
void
Camera::PushPose(std::unique_ptr<Device> &device,
                 struct pose_metrics &score,
                 struct xrt_pose &Tcv_cam_device,
                 ...
                 bool optimize)
{
	...
	auto Txr_world_cam = this->GetWorldPose(tbo.timestamp_ns);
	if (!Txr_world_cam.has_value()) {
...
CameraMosaic::CameraMosaic(ConstellationTracker *tracker,
                           const struct t_constellation_tracker_camera_mosaic &mosaic_params)
{
...
CameraMosaic::GetTrackingOriginPose(timepoint_ns when_ns)
{
	...
	return std::optional<struct xrt_pose>(XRT_POSE_IDENTITY);
}
...
Device::Device(struct t_constellation_tracker_device_params *params,
...
ConstellationTracker::ConstellationTracker(struct t_constellation_tracker_params *params)
{
...
ConstellationTracker::AddDevice(struct t_constellation_tracker_device_params *params,
                                struct t_constellation_tracker_device *device)
{
...
void
ConstellationTracker::MarkMatchingBlobs(struct t_blob_observation &bwobs,
